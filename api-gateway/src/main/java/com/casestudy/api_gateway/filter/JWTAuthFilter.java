...
public class JWTAuthFilter implements GlobalFilter, Ordered {
    ...
    public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
        ...
        if (openApiEndpoints.stream().anyMatch(path::contains)) {
            System.out.println("✅ Open API endpoint matched: " + path);
            return chain.filter(exchange);
        }
        ...
        if (isOpenApi(path)) {
            log.debug("Open API endpoint matched: {}", path);
            return chain.filter(exchange);
        }
        ...
        try {
            jwtUtil.validateToken(token);
            String role = jwtUtil.extractRole(token);
            log.debug("Token validated. Role: {}", role);

            if (!isAuthorized(path, role)) {
                log.warn("Forbidden: Role '{}' not allowed for path '{}'", role, path);
                exchange.getResponse().setStatusCode(HttpStatus.FORBIDDEN);
                return exchange.getResponse().setComplete();
            }
